<script>
  ...
  const options = {
    ...
    // ヘッダ表示 左/真ん中/右
    headerToolbar: {
      left: "customPrevYear,prev,next,customNextYear",
      center: "title",
      right: "today,dayGridMonth,timeGridWeek,timeGridDay,listWeek",
    },
    customButtons: {
      // ツールチップが正しく出ないためカスタムボタンで対応
      customPrevYear: {
        icon: "fc-icon-chevrons-left",
        hint: "前年へ移動",
        click: function () {
          calendar.getAPI().prevYear();
        },
      },
      customNextYear: {
    ...
    buttonHints: {
      ...
      next: "翌月へ移動",
      today: "今日の日付に移動",
      dayGridMonth: "月間ビューを表示",
      timeGridWeek: "週間ビュー（時間軸）を表示",
      timeGridDay: "日間ビュー（時間軸）を表示",
      listWeek: "週間リストビューを表示",
    },
  ...
  function handleDateClick(info) {
    ...
    if (clickedMonth === calendarMonth && clickedYear === calendarYear) {
      // クリックされた日付が現在の月の場合
    } else {
